...
function renderCustomReportGenerator(container, report) {
  ...
  container.innerHTML = `
    <div class="card" style="min-height:calc(100vh - 120px);margin-top:15px;overflow:hidden">
      <div class="card-hd" style="padding:16px 20px;background:#fff;border-bottom:1px solid #e2e8f0"><div style="display:flex;justify-content:space-between;gap:16px;align-items:center;flex-wrap:wrap"><div><div id="custom-report-title-display" class="card-title" style="font-size:18px;font-weight:800;color:#0f172a">${escapedName}</div><div class="card-sub" style="font-size:12px;color:#64748b;margin-top:4px">New replenishment workflow: Final DSR context + current survey evidence</div></div><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn btn-outline" onclick="window.showExistingReportsList()">Back</button><button class="btn btn-outline" onclick="window.updateCustomReportPreview(document.getElementById('custom-report-title-display').textContent, '${report.id}')">Refresh Preview</button><button class="btn btn-primary repl-download-files-btn" data-report-id="${report.id}" onclick="window.downloadCustomReportFilesDirect('${report.id}', this)">Download PDF + Word</button></div></div></div>
      <div style="padding:14px 20px;background:#eff6ff;border-bottom:1px solid #bfdbfe;display:flex;gap:12px;align-items:center;flex-wrap:wrap;color:#1e3a5f;font-size:12px"><strong>Source Final DSR:</strong><span>${replEscapeAttr(source)}</span><span style="background:#dcfce7;color:#166534;border-radius:999px;padding:3px 8px;font-weight:800">${inheritedCount} reusable fields</span><span style="background:#fef3c7;color:#92400e;border-radius:999px;padding:3px 8px;font-weight:800">${missingCount} items need review/upload</span></div>
      <div class="card-bd" style="display:grid;grid-template-columns:minmax(0,1fr) minmax(420px,.9fr);gap:18px;padding:18px;background:#f8fafc;align-items:start"><div style="min-width:0;display:flex;flex-direction:column;gap:14px"><div style="background:#fff;border:1px solid #dbe3ee;border-radius:12px;overflow:hidden;box-shadow:0 4px 14px rgba(15,23,42,.06)">${renderEnterpriseBuilderPanel(report)}</div><div style="background:#fff;border:1px solid #dbe3ee;border-radius:12px;overflow:hidden;box-shadow:0 4px 14px rgba(15,23,42,.06)">${renderFinalDsrUploadInventory(report)}</div></div><div style="display:flex;flex-direction:column;overflow:hidden;height:calc(100vh - 245px);min-height:560px;background:#fff;border-radius:12px;border:1px solid #cbd5e1;position:sticky;top:12px;box-shadow:0 4px 14px rgba(15,23,42,.08)"><div style="padding:10px 15px;background:#e2e8f0;border-bottom:1px solid #cbd5e1;display:flex;justify-content:space-between;align-items:center"><span style="font-weight:800;font-size:12px;color:#334155">Live Report Preview</span><span id="preview-sections-count" style="font-size:11px;background:#64748b;color:#fff;padding:2px 8px;border-radius:10px">Live</span></div><iframe id="custom-report-preview-iframe" title="Replenishment report preview" style="width:100%;height:100%;border:0;background:#fff;display:block" srcdoc="&lt;html&gt;&lt;body style='font-family:sans-serif;color:#64748b;padding:40px;text-align:center'&gt;Preparing preview...&lt;/body&gt;&lt;/html&gt;"></iframe></div></div>
    </div>`;
  updateCustomReportPreview(report.name || 'Replenishment Report', report.id);
  if (window.lucide) window.lucide.createIcons();
}
